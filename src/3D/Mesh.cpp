...
// internal implementation of the mesh
struct osc::Mesh::Impl final {
    ...
    std::optional<gl::Buffer> maybeVBO;  // GL_ARRAY_BUFFER
    std::optional<gl::Buffer> maybeEBO;  // GL_ELEMENT_ARRAY_BUFFER
...
static bool IsAnyValueGreaterThanU16Max(nonstd::span<uint32_t const> vs)
{
...
static std::unique_ptr<PackedIndex[]> RepackU32ToU16(nonstd::span<uint32_t const> vs)
{
...
static std::unique_ptr<PackedIndex[]> RepackU16ToU32(nonstd::span<uint16_t const> vs)
{
...
static std::unique_ptr<PackedIndex[]> CopyU32ToU32(nonstd::span<uint32_t const> vs)
{
    std::unique_ptr<PackedIndex[]> rv{new PackedIndex[vs.size()]};
...
static std::unique_ptr<PackedIndex[]> CopyU16ToU16(nonstd::span<uint16_t const> vs)
{
    int srcN = static_cast<int>(vs.size());
    int destN = (srcN+1)/2;
    std::unique_ptr<PackedIndex[]> rv{new PackedIndex[destN]};
...
static void* ConvertIntToVoidPtr(int v)
{
    ...
    m_Impl{new Impl{}}
{
    ...
    m_Impl->indexFormat = IsAnyValueGreaterThanU16Max(cpuMesh.indices) ? IndexFormat::UInt32 : IndexFormat::UInt16;
    m_Impl->numIndices = static_cast<int>(cpuMesh.indices.size());

    if (m_Impl->indexFormat == IndexFormat::UInt32)
    {
        m_Impl->indicesData = CopyU32ToU32(cpuMesh.indices);
    }
    else
    {
        m_Impl->indicesData = RepackU32ToU16(cpuMesh.indices);
    }
...
void osc::Mesh::setIndicesU16(nonstd::span<const uint16_t> vs)
{
    if (m_Impl->indexFormat == IndexFormat::UInt16)
    {
        m_Impl->indicesData = CopyU16ToU16(vs);
    }
    else
    {
        m_Impl->indicesData = RepackU16ToU32(vs);
    }
...
void osc::Mesh::setIndicesU32(nonstd::span<const uint32_t> vs)
{
    if (m_Impl->indexFormat == IndexFormat::UInt16)
    {
        m_Impl->indicesData = RepackU32ToU16(vs);
    }
    else
    {
        m_Impl->indicesData = CopyU32ToU32(vs);
    }
...
void osc::Mesh::uploadToGPU()
{
    ...
    // pack the CPU-side buffer for upload to the GPU
    ...
    // describe buffer layout in VAO

    int offset = 0;

    glVertexAttribPointer(SHADER_LOC_VERTEX_POSITION, 3, GL_FLOAT, false, static_cast<GLsizei>(stride), ConvertIntToVoidPtr(offset));
    glEnableVertexAttribArray(SHADER_LOC_VERTEX_POSITION);
    offset += 3 * sizeof(float);

    if (hasNormals)
    {
        glVertexAttribPointer(SHADER_LOC_VERTEX_NORMAL, 3, GL_FLOAT, false, static_cast<GLsizei>(stride), ConvertIntToVoidPtr(offset));
    ...
    if (hasUvs)
    {
        glVertexAttribPointer(SHADER_LOC_VERTEX_TEXCOORD01, 2, GL_FLOAT, false, static_cast<GLsizei>(stride), ConvertIntToVoidPtr(offset));
